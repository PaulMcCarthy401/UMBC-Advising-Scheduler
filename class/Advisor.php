...
class Appointment {
  ...
  public $id;
    ...
    public function __construct($fn, $ln, $un, $pw, $e) {
        $this->firstName = $fn;
        $this->lastName = $ln;
        $this->userName = $un;
        $this->password = $pw;
    ...
    public function addNewAdvisor() {
      ...
      $sql = "SELECT * FROM 'Advisor' WHERE 'Username' = '".$this->username."'";
      $result = $db->executeQuery($sql, "Add Advisor");

      if (mysql_num_rows($result) == 0){
	//if no rows returned, no username found, continue with insert

	$sql = "INSERT INTO `Advisor`(`ID`, `Username`, `Password`, `Email`, `Salt`) VALUES ('','".$this->userName."','".$this->password."','".$this->email."','')";
